...
if (!empty($member_GUID)) {
    $New_member = new member($_SESSION["Mechanic_GUID"]);
    ...
    $mem_License = $New_member->Get_MechanicLicense_ByID($member_GUID);
    ...
    // uncomment after 2024-04-12
     $inv = $Inv_fetch->getPayedTariff($member_GUID, $LastNTariffVer[0]['ID']);



    //$inv = $Inv_fetch->getPayedTariff($member_GUID, 4);

    if (!empty($inv)) {
        $tarrif = $Tariff_fetch->getMemberTariff($inv[0]['Title']);
        $data = $Tariff_Obj->V_NTariff_ByMechanicID($_SESSION["Mechanic_GUID"]);

        if (!empty($tarrif) || !empty($data)) {
                                        ...
                                        if ($internal == 1 && $external == 1) {
                                            ...
                                            foreach ($data as $key) {
                                                ...
                                                echo '<td>
                                                        ...
                                                        <input type="hidden" name="NTariffGUID" value="' . $key['GUID'] . '">
