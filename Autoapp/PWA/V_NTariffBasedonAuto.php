...
if (!empty($member_GUID) && !empty($_POST['AutoID'])) {
    ...
    if (!empty($Invoice)) {
        $tarrif = $Tariff_fetch->getMemberTariff_ByAutoID($Invoice[0]['Title'], $_POST['AutoID']);
        $data = $Tariff_Obj->V_NTariff_ByMechanicAutoID($_SESSION["Mechanic_GUID"], $_POST['AutoID'], $Invoice[0]['Title']);
