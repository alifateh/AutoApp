...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (file_exists($_FILES['fileToUpload']['tmp_name']) || is_uploaded_file($_FILES['fileToUpload']['tmp_name'])) {
            ...
            if ($uploadOk != 0) {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                    ...
                    //add with file
                    $route = $Banner_Obj->U_BannerImage_ByID($POST_BannerGUID, $target_file);
                } else {
                    $Error_STR = 1;
                }
            }
    ...
    if (isset($_POST['action']) && $_POST['action'] == "Edit-BannerImage") {
        if (isset($_POST['BannerGUID'])) {
            ...
            $str = '<h6 class="panel-title txt-dark"> ویرایش تصویر بنر در مکان : 
        [ ' . $position . ' ]
            </h6>';
        } else {
            $route = 1;
