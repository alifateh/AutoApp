...
if (!isset($_POST['Edit'])) {
	if (isset($_SESSION["Garage-ID"])) {
		...
		$skilles = $Garage_obj->GetGarageSkilles($Garage_GUID);
	} else {
		$url = "./V_GarageAll.php";
		header("Location: $url");
	}
...
if (isset($_POST['Edit'])) {
	...
	if (isset($_POST['optgroup'])) {
		$Garage_obj->U_GarageSkills_ByID($Garage_GUID, $_POST['optgroup']);
	} else {
		$Garage_obj->D_GarageSkills_ByID($Garage_GUID);
	}
														...
														<div class="row mt-40">
															<div class="col-sm-12">
																<select multiple id="optgroup" name="optgroup[]" multiple='multiple'>
																	...
																	if (!$skilles) {
																		$data = $Garage_obj->V_GarageSkills();
																		if(!empty($data)){
																			foreach ($data as $row) {
																		...
																		$data = $Garage_obj->V_GarageSkill_ByID($skilles);
																		if(!empty($data)){
																			foreach ($data as $row) {
																				$select = $Garage_obj->Selected($row['ID'], $skilles);
