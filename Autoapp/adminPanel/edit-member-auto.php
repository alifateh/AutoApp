...
if (!isset($_POST['Edit'])) {
	if (isset($_SESSION["GUID"])) {
		...
		$Mech_Autos = $auto_obj->Get_MechanicAuto_ByID($mem_GUID);
		$Mech_NotAutos = $auto_obj->Get_MechanicNOTAuto_ByID($mem_GUID);
	} else {
		$url = "./V_MemberAll.php";
		header("Location: $url");
	}
...
if (isset($_POST['Edit'])) {
	...
	if (isset($_POST['optgroup'])) {
		$auto = $_POST['optgroup'];
		$mem->U_MechanicAutomobile_ByID($mem_GUID, $_POST['optgroup']);
	} else {

		$auto[0] = 0;
		$mem->U_MechanicAutomobile_ByID($mem_GUID, $auto);
	}
																...
																<select multiple id="optgroup" name="optgroup[]" multiple='multiple'>
																	...
																	if (!empty($Mech_Autos)) {
																		...
																		foreach ($Manufacturer_All as $row) {
																			...
																			foreach ($Mech_NotAutos as $g_NoTcars) {
