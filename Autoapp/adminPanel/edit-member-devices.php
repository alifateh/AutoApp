...
if (!isset($_POST['Edit'])) {
	if (isset($_SESSION["GUID"])) {
		$mem = new member($_SESSION["GUID"]);
		$mem_GUID = $_SESSION["GUID"];
		$Devices = $mem->GetMemDevices($mem_GUID);
	} else {
		$url = "./V_MemberAll.php";
		header("Location: $url");
	}
...
if (isset($_POST['Edit'])) {
	$mem = new member($_POST['GUID']);
	$_SESSION["GUID"] = $_POST['GUID'];
	$mem_GUID = $_POST['GUID'];
	if (isset($_POST['optgroup'])) {
		$mem->U_MechanicDevices_ByID($mem_GUID, $_POST['optgroup']);
	} else {
		$Devices = array();
		$Devices[0] = 0;
		$mem->U_MechanicDevices_ByID($mem_GUID, $Devices);
	}
															...
															<div class="col-sm-12">
																<select multiple id="optgroup" name="optgroup[]" multiple='multiple'>
																	...
																	if (!$Devices) {
																		...
																		$mem->ViewDeviceSelectable($Devices);
																	}
																	?>
																</select>
															</div>
