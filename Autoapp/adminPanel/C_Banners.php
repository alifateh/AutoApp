...
$get_Banner = $Banner_Obj->Get_Banners_ByDate();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['Insert'])) {
        ...
        if (isset($_POST['P_Position']) && !empty($_POST['P_Position'])) {
            $Position = $_POST['P_Position'];
        } else {
            $Error_STR = 2;
        }

        if (isset($_POST['P_Slogn']) && !empty($_POST['P_Slogn'])) {
            $N_Slogn = $_POST['P_Slogn'];
        } else {
            $Error_STR = 2;
        }

        if (isset($_POST['P_Comments']) && !empty($_POST['P_Comments'])) {
            $N_Comments = $_POST['P_Comments'];
        } else {
            $Error_STR = 2;
        }

        if (isset($_POST['P_Owner']) && !empty($_POST['P_Owner'])) {
        ...
        if (isset($_POST['Post_VDate']) && !empty($_POST['Post_VDate'])) {
            $yesterday = date('Y-m-d', strtotime('-1 days', strtotime('2023-06-05')));
            ...
            $Date_End = jalali_to_gregorian($year, $mon, $day, '-');
        } else {
            $Error_STR = 2;
        }
        ...
        if (file_exists($_FILES['fileToUpload']['tmp_name']) || is_uploaded_file($_FILES['fileToUpload']['tmp_name'])) {
            ...
            $target_dir = "images/A_D_V/Banners/";
            ...
            if ($uploadOk != 0) {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                    //add with file
                    if (!in_array($Error_STR, $ER)) {
                        $route = $Banner_Obj->C_Banner($N_Link, $N_Slogn, $N_Comments, $N_Owner, $target_file, $Date_End, $Position);
                    }
                } else {
                    $Error_STR = 1;
                }
                                            ...
                                            <div class="col-sm-12">
                                                <div class="form-group">
                                                    <div class="input-group col-sm-6">
                                                        <label class="control-label mb-10" for="P_Position"> محل نصب بنر : </label>
                                                        <select class="form-control select2 select2-hidden-accessible" aria-hidden="true" name="P_Position" id="P_Position">
                                                            <?php
                                                            $pos = array();
                                                            if (!empty($get_Banner)) {
                                                                for ($i = 0; $i < 3; $i++) {
                                                                    $pos[$i] = $get_Banner[$i]['Position'];
                                                                }
                                                                $str = "";
                                                                if (!in_array(1, $pos)) {
                                                                    $str .= '<option value="1"> نخستین </option>';
                                                                }
                                                                if (!in_array(2, $pos)) {
                                                                    $str .= '<option value="2"> دومین </option>';
                                                                }
                                                                if (!in_array(3, $pos)) {
                                                                    $str .= '<option value="3"> سومین </option>';
                                                                }
                                                                if ($str == "") {
                                                                    echo '<option value="" selected> محل خالی برای نصب بنر موجود نیست </option>';
                                                                } else {
                                                                    echo $str;
                                                                }
                                                            } else {
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group col-sm-6">
                                                        ...
                                                        <input type="text" class="form-control" name="P_Slogn" id="P_Slogn" value="<?php echo $_POST['P_Slogn']; ?>">
                                                    </div>
                                                </div>
                                                <br />
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group col-sm-6">
                                                        <div class="input-group-addon"><i class="icon-info"></i></div>
                                                        <input type="text" class="form-control" name="P_Comments" id="P_Comments" value="<?php echo $_POST['P_Comments']; ?>">
                                                    </div>
                                                </div>
                                                <br />
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group col-sm-6">
                                                        <div class="input-group-addon"><i class="icon-info"></i></div>
                                                        <input type="text" class="form-control" name="P_Owner" id="P_Owner" value="<?php echo $_POST['P_Owner']; ?>">
                                                    </div>
                                                </div>
                                                <br />
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group col-sm-6">
                                                        <div class="input-group-addon"><i class="icon-info"></i></div>
                                                        <input type="text" class="form-control" name="P_Link" id="P_Link" value="<?php echo $_POST['P_Link']; ?>">
                                                    </div>
                                                </div>
                                                <br />
                                                <div class="form-group">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="fileinput input-group fileinput-new col-sm-6" data-provides="fileinput">
                                                ...
                                                <div class="form-group">
                                                    <div class="input-group col-sm-6">
                                                        <button class="btn btn-info btn-anim" type="submit" name="Insert" id="Insert"><i class="icon-check"></i><span class="btn-text">ثبت</span></button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
