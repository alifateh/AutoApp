...
if (isset($_POST['eltype']) && isset($_POST['ItemGUId'])) {
	$Etype = $_POST['eltype'];
	$ItemGUId = $_POST['ItemGUId'];
	$_SESSION["ItemGUId"] = $_POST['ItemGUId'];
}
...
<body>
									...
									<div id="example-basic">
										<div class="col-sm-6">
											<form method="post" enctype="multipart/form-data">
												<input type="hidden" id="elementype" name="elementype" value="<?php echo $Etype;  ?>">
												<input type="hidden" id="ItemGUId" name="ItemGUId" value="<?php echo $ItemGUId; ?>">
	...
	$elementype = $_POST["elementype"];
	$ItemGUId = $_POST["ItemGUId"];
	


	if ($elementype == 9) { //automobile photo
		...
		if (file_exists($_FILES['fileToUpload']['tmp_name']) || is_uploaded_file($_FILES['fileToUpload']['tmp_name'])) {
			...
			if (file_exists($target_file)) {
				...
				echo "$(document).ready(function() {
						...
						$.toast({
							...
							text: 'فایلی با این نام در سرور موجود می باشد'+'<br />'+'برای آپلود نام فایل را عوض کنید.' ,
							position: 'top-center',
							loaderBg:'#ed3236',
							hideAfter: 6500,
							stack: 6
			...
			if ($uploadOk == 0) {
				...
				if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $FileMove)) {
					$files->uploadfile($FileMove, $fileName, $elementype);
