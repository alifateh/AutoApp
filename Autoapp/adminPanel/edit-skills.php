...
use fateh\skills\skills;
	if (!isset($_POST['login-btn'])){
		if($_POST['action'] == "edit"){
			$ID = $_POST['skillsId'];
			$nameskills = new skills($_SESSION["Admin_GUID"]);
			$skill_name = $nameskills -> getkills($ID);
		} 
	}	
	...
	if (isset($_POST['login-btn'])) {
		...
		$skillname = $skill -> getkills($ID);
		$name = $_POST['skillname'];
			if ($name !== "" and $name !== $skillname){	
			$skill->updateskils($ID, $name);
			$url = "./view-skills.php";
			header("Location: $url");
				}else {
					$skill->updateskils($ID, $skillname);
					$url = "./view-skills.php";
					header("Location: $url");
				}
			}
									...
									<form method="post" enctype="multipart/form-data">
												<div id="example-basic">
													...
													<div class="form-group">
															...
															<input type="text" class="form-control" name="skillname" id="skillname" value="<?php echo $skill_name; ?>">
															<input type="hidden" id="skillsId" name="skillsId" value="<?php echo $ID; ?>"> 
															</div>
														</div>
														<br>
														<br>
