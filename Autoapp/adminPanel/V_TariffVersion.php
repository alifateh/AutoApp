...
if (isset($_POST['action'])) {
    if ($_POST['action'] == "remove") {
        $ID = $_POST['versionId'];
        $removeversion = new tariff($_SESSION["Admin_GUID"]);
        $removeversion->removeversion($ID);
        $url = "./view-version-tariff.php";
        header("Location: $url");
    }
}
                                        ...
                                        <div class="table-responsive">
                                            <table id="datable_1" class="table table-hover display  pb-30">
                                                ...
                                                <tbody>
                                                    ...
                                                    $viewversion = new tariff($_SESSION["Admin_GUID"]);
                                                    $data = $viewversion->V_TariffVersion();
                                                    if (!empty($data)) {
                                                        foreach ($data as $row) {
                                                            echo "<tr>";
                                                            echo "<td>" . $row['Name'] . "</td>";
                                                            ...
                                                            echo '<form method="post" enctype="multipart/form-data">
		                                                    	...
		                                                    	<form method="post" enctype="multipart/form-data" action ="edit-version-tariff.php">
		                                                    	<input type="hidden" name="versionId" value="' . $row['ID'] . '">
                                                        ...
                                                        echo '<tr><td colspan="3" style="text-align: center;"> نسخه ایی تعریف نشده است </td></tr>';
                                                    }
